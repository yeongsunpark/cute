...
import re
aa = re.compile('제\d+장')  # 1단계 (6개) / 숫자.숫자공백 으로 시작
bb = re.compile('제\d+조')  # 2단계 (5개) / 숫자.숫자.숫자공백 으로 시작
data = []
# 하기 전에 txt 파일이 utf-8로 인코딩이 되어 있나 꼭 확인하기!
# utf-8 파일은 제일 윗 줄을 제대로 인식을 못하니 한 줄 띄기!

temp_datas = [['' for _ in range(2)] for _ in range(2)]
...
with open("test.txt", 'r') as f:
    for line in f:
        ...
        if content_flag:  # 내용 부분에 관한 내용입니다.
            ...
            temp_datas[current_lvl][0] = line
            temp_datas[current_lvl][1] = ''

        if (content_flag or previous_lvl < current_lvl) and len(data) > 0:  # 만약에 content_flag가 맞거나 previous_lvl이 current_lvl보다 작고 data가  0 보다 크면.
            # print "data0", data[0]
            # print "data-1", data[-1]
            ...
            for i in range(current_lvl + 1, 6):
...
with open("hana_mrc_result.txt", "w") as f2:
   f2.write('\n'.join(data))