...
def syn_para(origin_utter):
    ...
    # 완성된 리스트의 중복 제거
    return list(set(pro_utter))

### 어미 다변화 ###
def eowrd_para(origin_utter):
    pro_utter = list()
    ...
    for utter in origin_utter:
        ...
        for eowrd_dict_key in eowrd_dict.keys():
            print utter.find(eowrd_dict_key)
            print len(utter) - len(eowrd_dict_key)
            print "----------------------"
            if utter.find(eowrd_dict_key) == len(utter) - len(eowrd_dict_key) -1:
...
if __name__ == "__main__":
    """
    origin_utter = syn_para(origin_utter)
    for utter in origin_utter:
        print utter
    
    origin_utter = we_para(origin_utter)
    for utter in origin_utter:
        print utter

    origin_utter = eowrd_para(origin_utter)
    for utter in origin_utter:
        print utter
    """
    origin_utter = eowrd_para(origin_utter)

    f = open("eowrd_para.txt", 'w')
    for utter in origin_utter:
        f.write(utter + '\n')
    f.close()
